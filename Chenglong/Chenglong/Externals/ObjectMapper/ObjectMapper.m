...
- (id)mapObject:(id)source toClass:(id)clazz withError:(NSError**)error {
  ...
  DLOG(@"Mapping time %f", [self timestamp] - startTimestamp);
...
- (void)map:(NSDictionary*)source toObject:(id)target error:(NSError**)error {
  ...
  for (NSString *key in [source allKeys]) {
    ...
    objc_property_t property = class_getProperty([target class], [key cStringUsingEncoding:NSUTF8StringEncoding]);
    
    if(property == NULL || [@"description" isEqualToString:key]) {
      ...
      property = class_getProperty([target class], [[@"_" stringByAppendingString:key] cStringUsingEncoding:NSUTF8StringEncoding]);
    }
    ...
    if (property == NULL) {
      ...
      if ([target respondsToSelector:NSSelectorFromString(selectorName)]) {
        ...
        property = class_getProperty([target class], [propertyName cStringUsingEncoding:NSUTF8StringEncoding]);
      }
    }
    ...
    // TODO: Add check for the type of the target property matching the value set?
      @try {
          if (targetPropertyValue) {
              NSString* forkey = [NSString stringWithCString:property_getName(property) encoding:NSUTF8StringEncoding];
              if ([target respondsToSelector:NSSelectorFromString(forkey)])
              {
                  [target setValue:targetPropertyValue forKey:forkey];
              }
              else {
                  NSLog(@"!!!!!!!!! undefined key %@", forkey);
              }
          }
...
+ (NSArray*)propertyNamesForClass:(Class)aClass {
	...
	if ((props = class_copyPropertyList(aClass, &propsCount))) {
		...
		while (propsCount--) 
			[results addObject:[NSString stringWithCString: property_getName(props[propsCount])	encoding: NSUTF8StringEncoding]];
		free(props);	
		return results;
  }
	return nil;			
...
+ (NSArray*)methodNamesForClass:(Class)aClass {
  ...
  if ((methods = class_copyMethodList(aClass, &methodCount))) {
    ...
    while (methodCount--) 
			[results addObject:[NSString stringWithCString: sel_getName(method_getName(methods[methodCount])) encoding: NSUTF8StringEncoding]];
    free(methods);	
    return results;
  }
  return nil;
